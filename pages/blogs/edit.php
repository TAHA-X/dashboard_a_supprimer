...
if(!isset($_SESSION["login"])){
  ...
  $error = "";
  $success = "";
  if(isset($_GET["num_blog"])){
     $blog = $conn->prepare("SELECT * from blogs where id=:num_blog");
     $blog->bindParam(":num_blog",$_GET["num_blog"]);
     $blog->execute(); 
     $blog = $blog->fetch();
  }
  if(isset($_POST["update"])){
       ...
       if($title!="" && $contenu!="" && $presentation!=""){
            if($img==""){
                $stmt = $conn->prepare("UPDATE blogs SET title=:title, presentation=:presentation, contenu=:contenu WHERE id=:num_blog");
            }
            else{
                unlink("imgs/".$blog["img"]);
                $stmt = $conn->prepare("UPDATE blogs SET title=:title, presentation=:presentation, contenu=:contenu,img=:img WHERE id=:num_blog");
            ...
            $stmt->bindParam(":num_blog", $id);
            $stmt->execute();
            $success = "update blog with success";
       }
       else{
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                      <div class="d-flex justify-content-between">
                            ...
                            <a href="index.php">consulter les blogs</a>
                      </div>
                      <?php
                                if($success != "") {
                      ...
                      <form enctype="multipart/form-data" class="pt-2" method="post">
                            <div class="w-100 m-2">
                                     ...
                                     <input value="<?php echo $blog["title"]; ?>" name="title" class="form-control"/>
                            </div>
                         
                              <div class="w-100 m-2">
                                     <label class="form-label">présentation</label>
                                     <textarea  class="form-control" name="presentation"><?php echo $blog["presentation"]; ?>
                                     </textarea>
                               </div>
                           ...
                           <div class="d-flex m-2 gap-2 w-100">
                               ...
                               <label class="form-label">img</label> <br/>
                                     <img src="imgs/<?php echo $blog["img"]; ?>" style="width:220px; height:210px;" />
