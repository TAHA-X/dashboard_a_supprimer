...
if(!isset($_SESSION["login"])){
  ...
  $error = "";
  $success = "";
  if(isset($_GET["num_service"])){
     $service = $conn->prepare("SELECT * from services where id=:num_service");
     $service->bindParam(":num_service",$_GET["num_service"]);
     $service->execute(); 
  ...
  if(isset($_POST["update"])){
       ...
       $img = $_FILES["img"]["name"];
       $date = date('Y-m-d');
       if($title!="" && $contenu!="" && $presentation!=""){
            if($img==""){
                $stmt = $conn->prepare("UPDATE services SET title=:title, presentation=:presentation, contenu=:contenu WHERE id=:num_service");
            }
            else{
                unlink("imgs/".$service["img"]);
                $stmt = $conn->prepare("UPDATE services SET title=:title, presentation=:presentation, contenu=:contenu,img=:img WHERE id=:num_service");
            ...
            $stmt->bindParam(":num_service", $id);
            $stmt->execute();
            $success = "update service with success";
       }
       else{
                ...
                <div class="container-fluid">
                      ...
                      <form enctype="multipart/form-data" class="pt-2" method="post">
                            <div class="w-100 m-2">
                                     ...
                                     <input value="<?php echo $service["title"]; ?>" name="title" class="form-control"/>
                            </div>
                         
                              <div class="w-100 m-2">
                                     <label class="form-label">présentation</label>
                                     <textarea  class="form-control" name="presentation"><?php echo $service["presentation"]; ?>
                                     </textarea>
                               </div>
                           ...
                           <div class="d-flex m-2 gap-2 w-100">
                               ...
                               <label class="form-label">img</label> <br/>
                                     <img src="imgs/<?php echo $service["img"]; ?>" style="width:220px; height:210px;" />
                           ...
                           <div class="d-flex m-2 gap-2 w-100">
                               <div class="w-100">
                                     ...
                                     <textarea id="editor" class="form-control" name="contenu">
                                        <?php echo $service["contenu"]; ?>
                                     </textarea>
                               </div>
                           </div>
                           <button name="update" class="btn m-2 btn-primary">update</button>
                      </form>
